#Πραγματικοί
#3. [Ιούνιος 2017- Ομάδα Εξέτασης 8] Γράψτε πρόγραμμα assembly MIPS που να διαβάζει συνεχώς σε ένα βρόχο πραγματικούς αριθμούς και βρίσκει αν είναι ακέραιοι. Όταν κάποιος αριθμός που διαβάζεται είναι ακέραιος τότε εκτυπώνει FOUND-INTEGER=X όπου Χ ο αριθμός. Διαφορετικά εκτυπώνει το μήνυμα NOT-INTEGER και προχωράει στην ανάγνωση του επόμενου. Αν διαβάσει το μηδέν τερματίζει.
...
.data
endl: 					.asciiz 	"\n"
#inp: .asciiz "Type a float: "
found: .asciiz "FOUND-INTEGER = "
notfound: .asciiz "NOT-INTEGER\n"
...
loop:
	...
	cvt.w.s $f12, $f0	# convert float ($f0) to int and store it to $f12
	mfc1 $s0, $f12		# move that int into $s0

	# CHECK
	beqz $s0, Exit		# If $s0 == 0, exit the program

	# CONVERT THE INT BACK TO FLOAT
	cvt.s.w $f13, $f12 # convert the converted-int into a float for comparison

	# COMPARE THE FIRST FLOAT WITH THE NEW FLOAT
	c.eq.s $f0, $f13
	# compare f0 (float) with f13 (its converted int, converted back to float)

	bc1t FOUND_INT			# if f0 == f13, go to FOUND INT
	bc1f NOT_FOUND_INT	# if f0 != f13 go to FOUND INT
