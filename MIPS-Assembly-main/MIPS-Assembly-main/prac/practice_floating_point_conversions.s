...
endl: 					.asciiz 	"\n"
NumA: .float 15.7
NumB: .double 123456789123456789.157123456789153
IntA: .word 0xAABBCCDD
IntB: .word 0xFACFDFBF
#################################################
#												#
#				text segment					#
#												#
...
#################################################
# NOTE: Just remember, if a number is lange and	#
# has to be stored in a DOUBLE register, it's	#
# actually 2 32-bit registers, eg: f0/f1, f2/f3	#
# etc.											#
#################################################
...
__start:
	# SINGLE TO INT
	l.s $f0, NumA		# Load NumA into $f0
	cvt.w.s $f0, $f0	# Convert it to integer
	mfc1 $t0, $f0		# Move the converted int to $t0

	move $a0, $t0		# Copy it to $a0 so it can be printed
	jal print_int_from_reg

	jal print_endl

	# DOUBLE TO INT
	l.d $f2, NumB		# Load NumB into $f2

	cvt.w.d $f2, $f2	# Convert it to a double
	mfc1 $t2, $f2		# Move the converted int to $t2

	move $a0, $t2		# Copy it to $a0 so it can be printed
	jal print_int_from_reg

	jal print_endl

	# INT TO SINGLE
	lw $t1, IntA		# Load IntA into $t1

	mtc1 $t1, $f1		# Move it to $f1, in the FPU
	cvt.s.w $f1, $f1	# Convert it to a single

	mov.s $f12, $f1		# Copy it to $f12 so it can be printed
	jal print_float_from_reg

	jal print_endl

	# INT TO DOUBLE
	lw $t4, IntB		# Load IntB into $t4

	mtc1 $t4, $f4		# Move it to $f4, in the FPU
	cvt.d.w $f4, $f4	# Convert it to a double

	mov.d $f12, $f4		# Copy it to $f12 so it can be printed
