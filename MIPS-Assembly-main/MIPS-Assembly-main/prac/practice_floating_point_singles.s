...
#################################################
	...
	.data
endl: 					.asciiz 	"\n"
typeint:	.asciiz "Type an integer: "
floatmess:	.asciiz "SINGLE SUM: "
#################################################
#												#
#				text segment					#
#												#
...
__start:
	...
	# f1 = converted float
	# f2 = an immediate (see below)
	# f3 = an immediate (see below)
	# f4 = sum of f1 + f2
	# f5 = sum of f1 + f3
	# f6 = sum of f4 + f5
	# PRINT SINGLE (FROM f6)


	la $a0, typeint			# Message to read int
	jal print_str

	jal read_int			# typed integers always go to $v0
	move $t0, $v0			# move the typed integer into $t0

	move $a0, $t0
	jal print_int_from_reg
	jal print_endl

	mtc1 $t0, $f0			# move the typed integer into $f0

	cvt.s.w	$f1, $f0		# convert the int (in $f0) into a single and place it into $f1

	li.s $f2, 19.100
	li.s $f3, 5943.466

	add.s $f4, $f1, $f2		# f4 = f1 + f2
	add.s $f5, $f1, $f3		# f5 = f1 + f3
	...
	mov.s $f12, $f6
	jal print_float_from_reg	# f12 always has the float to be printed
